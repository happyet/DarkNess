		...
		<div class="hentry translucent">
            ...
            <div class="hentry-content entry">
				...
				<?php 
					...
					$post_nums = $num_posts1->publish;
					
					echo '<h4 class="headline"><span>总共 ' . number_format_i18n( $post_nums ) . ' 篇文章</span></h4>';
					$args = array(
						'post_type' => array('post') , //如果你有多个 post type，可以这样 array('post', 'product', 'news')  
					...
					foreach ($posts_rebuild as $key => $value) {
						...
						foreach ($value as $key_m => $value_m) {
							$output .= '<dt class="archive-month"><a href="' . get_month_link( $year, $key_m ) . '">' . $key_m . ' 月</a> <span> + </span></dt><dd style="display:none;"><ol class="fancy-ul">';
